...
$env:CHAOXING_OUTPUT_DIR="D:\private\course-materials"
$env:CHAOXING_DISABLE_EXTENSIONS="1"       # optional, only when extensions interfere
```
...
The student must approve the output folder before download. The downloader stops when neither `--output` nor `CHAOXING_OUTPUT_DIR` is set.

The preferred portable route is direct HTTP fetch with browser cookies and PDF-header verification. Browser-native download or an already configured helper such as Xunlei/IDM may be used as a user-approved fallback, but the agent must explain the tradeoff and verify the final files. Extensions follow the automation profile by default; set `CHAOXING_DISABLE_EXTENSIONS=1` only when extensions interfere.
